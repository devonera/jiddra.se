...
var data = (function () {
  ...
  fn.go = function() {
    ...
    document.querySelector('.results').innerHTML = '<div class="loader"></div>';

    setTimeout(fn.ajax, 150);
  };
  ...
  fn.ajax = function(show_message = true) {
    ...
    fetch(fn.root + '/lan-api', {
      ...
      var count = results.querySelectorAll('.rows-list > li').length;
      console.log(count);

      if(show_message) {
        document.querySelector('.message').classList.add('is-open');
        document.querySelector('.message-button').innerHTML = count + ' st - TILL RESULTAT';
      }
      ...
      fn.eventClickResults();
    });
  };

  fn.eventClickResults = function() {
    document.querySelector('.message-button').addEventListener('click', fn.jumpResults, false);
  };

  fn.jumpResults = function() {
    var results = document.querySelector('.results');
    var offsets = results.getBoundingClientRect();
    ...
    document.querySelector('.message').classList.remove('is-open');
  };
